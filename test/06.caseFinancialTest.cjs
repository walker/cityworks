...
cw6.Cityworks.configure(process.env.cwdomain, {path: process.env.cwpath, version: process.env.cwversion});
const _ = require('lodash')

before(function(done) {
  this.timeout(20000000);
  cw6.Cityworks.authenticate(process.env.cwlogin, process.env.cwpassword).then(resp => {
    done();
  }).catch(e => {
...
describe('[CaseFinancial::makePayment] function test', () => {
  ...
  it('should make a payment on several fees', (done) => {
  ...
  it('should make payments on several fees', (done) => {
    cw6.briefcase.financial.makePayment([["ACH","117.72","test comment","2025-11-23 11:45:00","test"],["CC","108.00","test comment","2025-11-23 11:44:00","test"]], [263031,214310,269293]).then(r => {
      assert.isNumber(r);
      done();
    }).catch(e => {
      console.log(e, 'e')
